...
/// Converts a [`DateTime<Utc>`] to a PostgreSQL timestamp.
///
...
fn build_file_copy_buffer(files: &[FileRecord]) -> BytesMut {
    ...
    for file in files {
        ...
        put_field_i64(&mut buf, datetime_to_pg_timestamp(file.atime)); // atime
        put_field_i64(&mut buf, datetime_to_pg_timestamp(file.mtime)); // mtime
        put_field_i64(&mut buf, datetime_to_pg_timestamp(file.ctime)); // ctime
...
fn build_dir_copy_buffer(dirs: &[DirRecord]) -> BytesMut {
    ...
    for dir in dirs {
        ...
        put_field_i64(&mut buf, datetime_to_pg_timestamp(dir.mtime)); // mtime
        put_field_i64(&mut buf, now);                         // last_seen
    }
