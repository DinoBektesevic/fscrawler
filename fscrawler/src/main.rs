...
fn main() {
    ...
    if cli.create_tables {
        let url = config.db.url.as_deref().unwrap_or_else(|| {
            eprintln!("error: --db-url required when --create-tables provided");
            std::process::exit(1);
        });
    ...
    if cli.clear {
        let url = config.db.url.as_deref().unwrap_or_else(|| {
            eprintln!("error: --db-url required when --clear provided");
            std::process::exit(1);
        });
